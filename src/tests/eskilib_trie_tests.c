...
void eskilib_trie_search_no_results_test(void) {
	...
	struct eskilib_Trie* search_result = eskilib_trie_search(string_search, tree);
	eskilib_assert(search_result == NULL);
...
void eskilib_trie_matches_no_results_test(void) {
	...
	struct eskilib_Trie* search_result = eskilib_trie_search((struct eskilib_String){ .value = "ss", .length = 3 }, tree);
	eskilib_assert(search_result != NULL);
