...
void eski_test_run(const char* function_name, void (*function)(void))
{
	test_failed = false;
	printf("%s started.\n", function_name);
...
void eski_test_failed_internal(void) {
	test_failed = true;
}

void eski_test_start_internal(char* file) {
	test_failed = 0;
	tests_passed = 0;
	printf(YELLOW_BRIGHT "Starting tests for %s\n" RESET, file);
}

void eski_test_finish_internal(char* file) {
